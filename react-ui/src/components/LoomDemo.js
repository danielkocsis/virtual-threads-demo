...
function LoomDemo() {
  ...
  const fetchAuditCount = useCallback(async () => {
    ...
    try {
      const response = await fetch('/api/demo/loom');
      if (!response.ok) {
  ...
  const handleProcessRequest = async () => {
    setIsLoading(true);
    setMessage('Processing Loom request...');
    try {
      const response = await fetch('/api/demo/loom', { method: 'POST' });
      if (!response.ok) {
        const errorData = await response.text();
        throw new Error(`HTTP error! status: ${response.status}, details: ${errorData}`);
      }
      const data = await response.json();
      setMessage(`Loom request processed successfully! Response: ${data.message}`);
      fetchAuditCount(); // Refresh count
    } catch (error) {
      console.error("Failed to process Loom request:", error);
      setMessage(`Failed to process Loom request: ${error.message}`);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <>
      ...
      <button onClick={handleProcessRequest} disabled={isLoading}>
        {isLoading ? 'Processing...' : 'Process Request (Loom)'}
      </button>
